...
int main( )
{
    ...
    size_t datatoread = 0, dataread = 0, datalose = 0;
                ...
                if ( event->mask & IN_MODIFY ) {
                    if ( !(event->mask & IN_ISDIR) ) {
                                ...
                                if(pos == MAXF ){
                                    ...
                                    *sp = '\0';
                                    ...
                                    if(!strcmp(event->name , mname)){
                                        ...
                                        if(cfile == NULL){
                                            printf("Data to count %d  diff %lu read %lu No file for campaign %s \n",count, ep - sp,datatoread,  _feed.campaign);
                                        }
                                        else{
                                            ...
                                            fprintf(cfile,"%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n",_feed.prodId, _feed.prodName, _feed.landingPage, _feed.imageUrl, _feed.catName, _feed.catId, _feed.discountPrice, _feed.offerPrice, _feed.blank, _feed.description, _feed.subCat1Id, _feed.subCat1Name, _feed.subCat2Id, _feed.subCat2Name, _feed.misc1, _feed.misc2, _feed.campaign);
                                        }
                                    }
                                    pos = 0;
                                    *(&feed[pos]) = sp+1;
                        ...
                        if(!strcmp(event->name , mname)){
                            ...
                            f2sizeold = f2sizenew - datalose;
                        }
                        else if(!strcmp(event->name , fname)){ 
                            fsizeold = fsizenew - datalose;
                        }
